...
physical_read_item_mms_ids = ['9930488810501221', '9930488809701221', '9930488809101221']

multiple_courses.each do |course|

    puts "Retrieivng reading list #{course[:reading_list_id]}"
    ...
    puts "Finished reading list #{course[:reading_list_id]}"

end


puts "Weighted citations count: #{weighted_citations.count}"

puts weighted_citations

puts "Removing physical read items"

physical_read_item_mms_ids.each do |mms_id|

    key = "MMS_#{mms_id}"

    weighted_citations.reject! { |citation| citation[:key] == key }
